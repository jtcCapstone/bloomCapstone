...
export class LlmService {
  ...
  constructor() {
    ...
    this.apiUrl = process.env.LLM_API_URL || 'http://vm-ip:11434/api/generate';
  ...
  async generateResponse(
    ...
    try {
      ...
      this.logger.log(
        `Starting request - Prompt: "${prompt.substring(
          0,
          30,
      ...
      this.logger.log(
        `Prompt built - Length: ${fullPrompt.length} - Time: ${
          Date.now() - promptBuildStart
        }ms`,
      );
      ...
      this.logger.log(
        `Raw LLM response (${apiCallTime}ms): "${rawResponse.substring(
          0,
          100,
        )}..."`,
      );
      ...
      cleanedResponse = cleanedResponse
        .split(/\n(Human|User|AI|Assistant):/i)[0]
        .trim();

      // Remove meta-commentary about the conversation
      cleanedResponse = cleanedResponse.replace(
        /^(Yes|No),?\s*(the human|the user).{0,100}(provided|asked|said|mentioned).{0,200}/i,
        '',
      );
      cleanedResponse = cleanedResponse.replace(
        /^(This|The)\s+(conversation|message|text|prompt).{0,200}/i,
        '',
      );
      cleanedResponse = cleanedResponse.replace(
        /^(Based on|According to).{0,50}(conversation|message|text).{0,100}/i,
        '',
      );

      // Remove references to AI's limitations
      cleanedResponse = cleanedResponse.replace(
        /^(I|As an AI).{0,50}(don't|cannot|can't|am unable to).{0,100}/i,
        '',
      );
      cleanedResponse = cleanedResponse.replace(
        /^The\s+AI\s+assistant\s+(does not|cannot|is unable to).{0,100}/i,
        '',
      );
      ...
      this.logger.log(
        `Cleaned response: "${cleanedResponse.substring(0, 100)}..."`,
      );

      // Provide fallback if we've removed too much
      if (cleanedResponse.trim().length < 15) {
        cleanedResponse =
          "I'd be happy to help with that. What specific information would you like?";
        this.logger.warn(
          'Using fallback response due to short/empty cleaned response',
        );
      }

      const totalTime = Date.now() - startTime;
      this.logger.log(
        `Total request time: ${totalTime}ms - API: ${apiCallTime}ms (${Math.round(
          (apiCallTime / totalTime) * 100,
        )}%), Post-processing: ${Date.now() - postProcessStart}ms`,
      );
